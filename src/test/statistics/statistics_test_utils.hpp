...
#define EXPECT_COLUMN_STATISTICS_IMPL(column_statistics_expr, null_value_ratio_, distinct_count_, min_, max_,       \
  ...
  {                                                                                                                 \
    ...
    min_max_cmp(std::dynamic_pointer_cast<const MinimalColumnStatistics<data_type_type>>(column_statistics)->min(), \
                min_);                                                                                              \
    min_max_cmp(std::dynamic_pointer_cast<const MinimalColumnStatistics<data_type_type>>(column_statistics)->max(), \
                max_);                                                                                              \
  }
