 ...
 private:
  ...
  class SingleColumnTypeIterator : public BaseColumnIterator<SingleColumnTypeIterator<ConcreteColumnType>, ColumnIteratorValue<T>> {
   ...
   private:
    ...
    bool equal(const SingleColumnTypeIterator& other) const { return _pos_list_it == other._pos_list_it; }
   ...
   private:
    void _insert_referenced_column(const ChunkID chunk_id) {
      if (_columns[chunk_id] == nullptr) {
        auto column = _table->get_chunk(chunk_id)->get_column(_column_id);
        auto concrete_column = std::dynamic_pointer_cast<const ConcreteColumnType>(column);
        DebugAssert(concrete_column, "Cannot cast to concrete column type");
        _columns[chunk_id] = concrete_column;
      }
    }
   ...
   private:
    ...
    std::vector<std::shared_ptr<const ConcreteColumnType>> _columns;
  };
};
